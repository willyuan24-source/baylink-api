...
async function callPlannerAi({ config, ai, isTest, message, inferred, explicit, today, catalog, locale }) {
  const payload = { message, filters: { ...inferred, ...explicit }, currentDatePacific: today, locale,
    events: catalog.events.filter(row => active(row) && nextEventDate(row, today) !== null).slice(0, 160).map(row => ({ id: row.id, title: row.title, startDate: row.startDate, endDate: row.endDate, occurrenceDates: row.occurrenceDates, region: row.region, city: row.city, category: row.category, cost: row.cost, planning: row.planning })) };
...
async function recommend({ body, catalog, now = Date.now, config = {}, ai, isTest = false }) {
  ...
  eligible.sort((a, b) => {
    ...
    return knownCost || rank(a) - rank(b) || family || nextEventDate(a, today).localeCompare(nextEventDate(b, today)) || a.id.localeCompare(b.id);
  });
...
module.exports = { ID, REGIONS, TRAVEL, plain, validDate, text, fail, active, eventOccursOn, loadPlannerCatalog, validateFilters, inferFilters, priceOf, distanceKm, recommend };
