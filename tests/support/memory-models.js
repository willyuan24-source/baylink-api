  ...
  return Object.entries(query).every(([key, expected]) => {
    ...
    if (expected && typeof expected === 'object' && !Array.isArray(expected)) {
      return Object.entries(expected).every(([operator, operand]) => {
        ...
        if (operator === '$nin') return !operand.includes(value);
...
function memoryModel(name, seed = []) {
  ...
  return {
    ...
    create: async value => new Document({ createdAt: Date.now(), isDeleted: false, likes: [], comments: [], reports: [], ...value }).save(),
    updateOne: async (query, update, options = {}) => {
      const index = rows.findIndex(row => matches(row, query));
      if (index >= 0) Object.assign(rows[index], copy(update.$set || {}));
      else if (options.upsert) rows.push({ ...copy(query), ...copy(update.$setOnInsert || {}), ...copy(update.$set || {}) });
      return { acknowledged: true };
    },
    updateMany: async (query, update) => { rows.filter(row => matches(row, query)).forEach(row => Object.assign(row, copy(update.$set || update))); return { acknowledged: true }; },
    findOneAndUpdate: async (query, update, options = {}) => {
      let row = rows.find(row => matches(row, query));
      if (!row && options.upsert) { row = copy(query); rows.push(row); }
      if (!row) return null;
      Object.assign(row, copy(update.$set || update));
